...
object DataHelper {
    ...
    private fun loadItems(): ArrayList<Item> {
        ...
        itemList.apply {
            ...
            add(Item(list = arrayListOf(TextWithSwitchV(TextWithSummaryV(titleResId = R.string.smallFolderBlur), "folderBlur", customOnCheckedChangeListener =  { _, _ -> currentActivity.recreate() }))))
            if (OwnSP.ownSP.getBoolean("folderBlur", false)) add(Item(list = arrayListOf(TextWithSeekBarV(TextV(resId = R.string.smallFolderBlurCorner), key = "folderBlurCorner", min = 0, max = 50, divide = 1, defaultProgress = 40))))
