...
class TitleTextV(val text: String? = null, val resId: Int? = null, private val onClickListener: View.OnClickListener? = null): BaseView() {
    ...
    override fun create(context: Context): View {
        return TextV(text, resId, sp2px(context,4.5f), Color.parseColor("#9399b3"), padding = Padding(dp2px(context, 25f), dp2px(context, 14f), 0, dp2px(context, 14f)), onClickListener = onClickListener).create(context)
    }
