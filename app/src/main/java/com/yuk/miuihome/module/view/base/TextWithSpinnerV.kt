...
class TextWithSpinnerV(private val textV: TextV, val key: String, var select: String?, val array: ArrayList<String>, private val callBacks: ((String) -> Unit)? = null): BaseView() {
    ...
    override fun create(context: Context): View {
        ...
        val spinner = LinearContainerV(
            ...
            arrayOf(
                LayoutPair(text.also { it.text = select; it.setPadding(0, 0, 0, 0); it.textSize = sp2px(context, 5.5f) }, LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT, 1f).also { it.gravity = Gravity.CENTER_VERTICAL + Gravity.RIGHT}),
                LayoutPair(ImageView(context).also { it.background = context.getDrawable(R.drawable.ic_up_down) }, LinearLayout.LayoutParams(dp2px(context, 22f), dp2px(context, 22f)))
            )
        )
        return LinearContainerV(
            ...
            arrayOf(
                ...
                LayoutPair(spinner.create(context).also { it.setPadding(0, 0, dp2px(context, 19f), 0) }, LinearLayout.LayoutParams(LinearLayout.LayoutParams.WRAP_CONTENT, LinearLayout.LayoutParams.WRAP_CONTENT).also { it.gravity = Gravity.CENTER_VERTICAL })
            )
